  ...
  const contactDetails = [
    {
      ...
      title: 'Address',
      lines: ['50 NW 23rd St. Suite 112', 'Miami, FL 33127']
    },
    {
      icon: (
        <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-8 py-12 border-t border-gray-200">
      {contactDetails.map((detail, index) => (
        <div key={index} className="text-center">
          ...
          <h3 className="text-xl font-recoleta font-bold text-surenitea-700 mb-2">
            {detail.title}
          </h3>
          {detail.lines.map((line, lineIndex) => (
            <p key={lineIndex} className="text-surenitea-600 font-sofia">
              {line}
            </p>
          ))}
        </div>
